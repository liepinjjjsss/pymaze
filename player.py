...
class Player:
    ...
    def check_collision(self, grid_cells, thickness):
        current_cell_x, current_cell_y = self.x // TILE_SIZE, self.y // TILE_SIZE
        current_cell = self.get_current_cell(current_cell_x, current_cell_y, grid_cells)
        current_cell_abs_x, current_cell_abs_y = current_cell_x * TILE_SIZE, current_cell_y * TILE_SIZE
        if self.left_pressed:
            if current_cell.walls['left']:
                if self.x <= current_cell_abs_x + thickness:
                    self.left_pressed = False
        if self.right_pressed:
            if current_cell.walls['right']:
                if self.x >= current_cell_abs_x + TILE_SIZE - (self.player_size + thickness):
                    self.right_pressed = False
        if self.up_pressed:
            if current_cell.walls['top']:
                if self.y <= current_cell_abs_y + thickness:
        ...
        if self.down_pressed:
            if current_cell.walls['bottom']:
                if self.y >= current_cell_abs_y + TILE_SIZE - (self.player_size + thickness):
                    self.down_pressed = False
